...
func applyKeyValueDelta(state *basics.TealKeyValue, key []byte, vd basics.ValueDelta, reverseDelta *idb.AppReverseDelta) (err error) {
	if *state == nil {
		*state = make(map[string]basics.TealValue)
	}

	oldValue, ok := (*state)[string(key)]
	if ok {
		switch oldValue.Type {
		case basics.TealUintType:
			reverseDelta.SetDelta(key, basics.ValueDelta{Action: basics.SetUintAction, Uint: oldValue.Uint})
		case basics.TealBytesType:
			reverseDelta.SetDelta(key, basics.ValueDelta{Action: basics.SetBytesAction, Bytes: oldValue.Bytes})
		default:
			return fmt.Errorf("old value key=%s ov.T=%T ov=%v", key, oldValue, oldValue)
		}
	} else {
		reverseDelta.SetDelta(key, basics.ValueDelta{Action: basics.DeleteAction})
	}
	newValue := oldValue
	switch vd.Action {
